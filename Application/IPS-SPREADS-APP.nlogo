...
to setup
  ...
  if path-to-input = "input/A/" [
    ; impose a worst case scenario
    set winddirection 315

    ; correct errors from rasterisation of shapefiles
    ask spruce with [spruceprop = 0.34855919852378087 and inf = 4][set inf 1]
    ask spruce with [spruceprop = 1 and inf = 4][set inf 1]
    ask spruce with [spruceprop = 0.579999983310699 and inf = 4][set inf 1]
    ask spruce with [spruceprop = 0.449999988079071 and inf = 4][set inf 1]
    ask spruce with [spruceprop = 0.550000011920929 and inf = 4][set inf 1]
    ask spruce with [spruceprop = 0.75 and inf = 4][set inf 1]
  ]

  if path-to-input = "input/B/" [
    ; impose a worst case scenario
    set winddirection 135

    ; correct errors from rasterisation of shapefiles
    ask spruce with [spruceprop = 1 and inf = 4][set inf 1]
  ]

  if path-to-input = "input/C/" [
    ; impose a worst case scenario
    set winddirection 180

    ; correct error from rasterisation of shapefiles
    ask spruce with [pxcor < 50 and inf = 4][set inf 1]
    ask spruce with [pxcor > 120 and pycor < 110][set inf 1]
    ask patch 51 145 [ask spruce in-radius 4 with [inf = 4][set inf 1]]

    ; remove further infestation cohorts
    ask source with [pycor < 90][
      set source (source with [self != myself])
      set pcolor black
    ]
  ]

  if path-to-input = "input/D/" [
    ; impose a worst case scenario
    set winddirection 45

    ; remove further infestation cohorts
    ask source with [pxcor < 50 or pxcor > 120] [
      set source (source with [self != myself])
      set pcolor black
    ]
  ]
  ...
  if path-to-input = "input/E/" [
    ...
    ask source with [pycor < 90 or pycor > 120][
      set source (source with [self != myself])
      set pcolor black
    ]
  ]
  ...
  if ((mean [wave-count] of source) = swarming) [ ; check if all sources deployed their last flightwave
    if (beetle_dispersing = 0) and (beetle_staying = 0)  [ ; check if any beetles are flying or staying
      ...
      ifelse (ngeneration = generations) or (count host = 0) [ ; check if maximum number of generations is reached or if no trees were infested
        ;export-view (word "output/" behaviorspace-run-number ".png")
        stop ; end simulation
      ][
        set day (day + 1)
        draw-plots
...
to import-GIS-data
  ...
  ifelse max-pxcor > max-pycor [
    set-patch-size (702.5 / max-pxcor) ; set displayed patch size in dependency of the given GIS data for similar NetLogo world sizes
  ][
    set-patch-size (702.5 / max-pycor) ; set displayed patch size in dependency of the given GIS data for similar NetLogo world sizes
  ]
...
to cut-infested-trees
  ...
  let sfnA round ((count host with [inf != 4]) * sanitation-fellingA / 100) ; calculate number of trees to be cut inside nlp
  ask n-of sfnA host with [inf != 4] [
  ...
  let sfnB round ((count host with [inf = 4]) * sanitation-fellingB / 100) ; calculate number of trees to be cut outside nlp
  ask n-of sfnB host with [inf = 4] [
    set host (host with [self != myself]) ; remove felled trees from hosts
    set beetle_infesting (beetle_infesting - [ninf] of self) ; remove beetles in felled trees from infesting beetles
    set beetle_killed (beetle_killed + [ninf] of self) ; add beetles in felled trees to killed beetles
    set pcolor yellow
...
to calculate-results
  ...
  if ngeneration = generations or count host = 0 [
    ...
    set ncut1 (count patches with [pcolor = yellow and inf != 4])
    set ncut2 (count patches with [pcolor = yellow and inf = 4])

    set nadj (count patches with [pcolor = red and inf = 4])
    set nnlp (count patches with [pcolor = red and inf != 4])
...
@#$#@#$#@
<experiments>
  <experiment name="APP" repetitions="40" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>nhost</metric>
    <metric>ncut1</metric>
    ...
    <enumeratedValueSet variable="sanitation-fellingA">
      <value value="0"/>
      <value value="25"/>
      <value value="50"/>
      <value value="75"/>
    ...
    <enumeratedValueSet variable="sanitation-fellingB">
      <value value="0"/>
      <value value="25"/>
      <value value="50"/>
      <value value="75"/>
  ...
  <experiment name="APP-A" repetitions="40" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>nhost</metric>
    <metric>ncut1</metric>
  ...
  <experiment name="APP-B" repetitions="40" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>nhost</metric>
    <metric>ncut1</metric>
  ...
  <experiment name="APP-C" repetitions="40" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>nhost</metric>
    <metric>ncut1</metric>
  ...
  <experiment name="APP-D" repetitions="40" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>nhost</metric>
    <metric>ncut1</metric>
  ...
  <experiment name="APP-E" repetitions="40" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>nhost</metric>
    <metric>ncut1</metric>
